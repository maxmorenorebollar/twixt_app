...
export function initGraph(): Graph {
  ...
  nodes.push({ ...initNode(576, -1, -1), player: 0 }); // top node connect to player 0
  nodes.push({ ...initNode(577, -1, -1), player: 0 }); // bot node connect to player 0
  nodes.push({ ...initNode(578, -1, -1), player: 1 }); // left node connect to player 1
  nodes.push({ ...initNode(579, -1, -1), player: 1 }); // right node connect to player 1
...
export function findWinner(graph: Graph) {
  const visited = new Set<number>();

  // Check if player 0 (top/bottom) has won
  const dfsPlayer0 = (current: number): boolean => {
    if (current === 577) {
      // Reached bottom node
      return true;
    }
  ...
  const dfsPlayer1 = (current: number): boolean => {
    if (current === 579) {
      // Reached right node
      return true;
    }
  ...
  visited.clear();
  if (dfsPlayer0(576)) return 0;

  // Check player 1 (left to right)
  visited.clear();
  if (dfsPlayer1(578)) return 1;
