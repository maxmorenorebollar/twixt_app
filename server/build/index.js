...
const gameStateManager = new Map();
app.get("/", (_req, res) => {
    res.send("Pong");
});
...
io.on("connection", (socket) => {
    socket.on("join-game", ({ playerId, gameId }) => {
        ...
        const players = [...((_a = gameManager.get(gameId)) !== null && _a !== void 0 ? _a : [])];
        if (players.length == 2) {
        ...
        if (!games) {
            throw new Error("join-game: game could not be found in gamestate manager");
        }
    ...
    socket.on("end-turn", ({ gameId, playerId, gameState }) => {
        ...
        if (newGameState.player === undefined) {
            throw new Error("end-turn: player can't make move game with undefined player");
        }
